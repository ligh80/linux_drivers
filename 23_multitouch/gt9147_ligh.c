...
static irqreturn_t gt9147_irq_handler(int irq, void *dev_id)
{
     ...
     */
	/* 读取GT1151触摸点坐标从第一个寄存器开始，连续读取40个寄存器，每一个点8个字节 */
	ret = gt9147_read_regs(dev, GT_TP1_REG, multouch_data, 40);
	if (ret) {
		goto fail;
	}
	for (i = 0; i < MAX_SUPPORT_POINTS; i++){
		touch_data = &multouch_data[8 * i];
