...
{
	/* 通过判断原子变量的值来检查LED有没有被其他的应用使用,atomic_dec_and_test()返回一个整型值，如果原子类型v在减1后变为0，则返回1，否则返回0 */
	if(!atomic_dec_and_test(&gpioled.lock)){
		atomic_inc(&gpioled.lock);		/* 因为在if的判断语句有做减1的操作，这里使其回复原来的值 */
		return -EBUSY;
	}
...
{
	struct gpioled_dev *dev = filp->private_data;
	atomic_inc(&dev->lock);	/* 关闭驱动文件的时候，释放原子变量 */
	return 0;
}
